...
def importfits(fitsimage,imagename,whichrep,whichhdu,zeroblanks,overwrite,defaultaxes,defaultaxesvalues,beam):
    ...
    try:
        ...
        if beam!=[]: # user has set beam
            ...
            for i in range(0,3):
                try:
                    qabeam.append(_myqa.quantity(beam[i]))
        ...
        if addaxes:
            ...
            _myia.open(tmpname)
            ia2  = _myia.adddegaxes(outfile=tmpname2, direction=True, spectral=True, stokes=defaultaxesvalues[3],silent=True)
            _myia.close( )
            os.system('rm -rf '+tmpname)
            ia2.close()
            ia2.open(tmpname2)
            ...
            if adddir:
                ...
                if type(ra)==int or type(ra)==float:
                    ...
                    qara = _myqa.quantity(_myqa.angle(ra)[0])
                ...
                if type(dec)==int or type(dec)==float:
                    ...
                    qadec = _myqa.quantity(_myqa.angle(dec)[0])
