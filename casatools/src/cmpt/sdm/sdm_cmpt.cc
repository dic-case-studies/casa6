...
//
static string lrtrim(std::string& s,const std::string& drop = " ") {
...
inline static float d2f(double d) { return (float) d; }
inline static double m2au(double m) {
    const static double au_m = casacore::Quantity(1.0, "AU").getValue("m");  // AU in meters
    return m / au_m;
}
inline static double mpers2auperd(double mpers) { return m2au(mpers) * 24. * 3600.; }
inline static double radian2degree(double radian) { return radian / M_PI * 180.0; }
inline static bool notNull(int n) { return n != 0 ; }
...
static void linearInterpCoeff(uint32_t npoints, const vector<double> &time_v, const vector<double> &k_v, vector<vector<double> > &coeff_vv) {
...
static void myTimer( double *cpu_time ,   /* cpu timer */
 ...
 */
static void rect(const vector<double>& s, vector<double>& x) {
 ...
 * Attention ! a and x must be correctly sized !
 */
static void spher(const vector<double>& x, vector<double>& s) {
 ...
 */
static void topo2geomat(double lambda, double phi, vector<vector<double> >& mat) {
 ...
 */
static void matvec (const vector<vector<double> >& mFactor, const vector<double>& vFactor, vector<double>& vResult) {
...
class StokesMapper {
private :
  casacore::Stokes::StokesTypes* sa;

public :
  StokesMapper();
  ~StokesMapper();

  static casacore::Stokes::StokesTypes value(StokesParameterMod::StokesParameter s);
  casacore::Stokes::StokesTypes* to1DArray(const vector<StokesParameterMod::StokesParameter>& v);
  static vector<casacore::Stokes::StokesTypes> toVectorST(const vector<StokesParameterMod::StokesParameter>& v);
...
casacore::Stokes::StokesTypes* StokesMapper::to1DArray(const vector<StokesParameterMod::StokesParameter>& v) {
...
class SConverter {
public :
    template<class Enum, class EnumHelper> static vector<string> toVectorS(const vector<Enum>& vEnum) {
...
static vector<Tag> reorderSwIds(const ASDM& ds) {
...
public:
    MSMainRowsInSubscanChecker(const casac::sdm &o) : obj(o) { }
    virtual ~MSMainRowsInSubscanChecker();
    void check(const sdmbin::VMSData* vmsData_p, MainRow* mainRow_p, unsigned int mainRowIndex, const string& BDFName);
...
void MSMainRowsInSubscanChecker::check( const sdmbin::VMSData* vmsData_p,
                                        MainRow* mainRow_p,
...
const vector<string>& MSMainRowsInSubscanChecker::report() const { return report_v; }


namespace casac {

    static vector<int>	dataDescriptionIdx2Idx;

    sdm::sdm( const std::string &path ) {
        sdm_path = path;
    }

    bool sdm::toms( const std::string &vis, bool createmms, const std::string &separationaxis, const variant &numsubms,
                    ...
                    bool flagbackup, bool verbose, bool overwrite,
        ...
        if ( gen_ms( vis, createmms, separationaxis, numsubms, corr_mode, srt, time_sampling, ocorr_mode, compression,
                     ...
                     overwrite, bdfflags, with_pointing_correction,
                     convert_ephem2geo, polyephem_tabtimestep ) ) {
            if ( bdfflags ) {
                bool uncorrected_data = false;
                if (wvr_corrected_data.compare("no") == 0)
                    uncorrected_data = true;
                return bd_flagger( vis, ocorr_mode, scans, lazy && ! compression, uncorrected_data );
            } else return true;
        } else return false;
    }

    bool sdm::gen_ms( const std::string &vis, bool createmms, const std::string &separationaxis, const variant &numsubms,    
                      const std::string &corr_mode, const std::string &srt, const std::string &time_sampling,
                      const std::string &ocorr_mode, bool compression, bool lazy, const std::string &asis,
                      const std::string &wvr_corrected_data, const std::string &scans, bool ignore_time,
                      bool process_syspower, bool process_caldevice, bool process_pointing, bool process_flags,
        ...
        try {
            ...
            ASIS                       ---> asis
            WVRCORRDATA                ---> wvr_corrected_data
            SCANS                      ---> scans
            LOGFILE                    --->
            VERBOSE                    ---> verbose
            DRYRUN                     ---> NULL
            IGNORETIME                 ---> ignore_time    
            NOCALDEV                   ---> not process_caldevice
            NOEPHMERIS                 ---> NULL
            NOSYSPOWER                 ---> not process_syspower 
            NOPOINTING                 ---> not process_pointing
            CHECKROWUNIQ               ---> NULL
            BDFSLICESIZE               ---> 500
            LAZY                       ---> lazy
            WITHPCORR                  ---> with_pointing_correction
            ACXCPERTIME                ---> "no"
            POLYEPHTSTEP               ---> polyephem_tabtimestep
            INTEPHEM                   ---> "no"
            ASDMDIR                    ---> sdm_path
            MSDIRPREFIX                ---> ms
            CHECKDUPINTS               ---> "true"
            ********************************************************************************/
            ...
            while (iss >> token) {
                if (token.compare("co") == 0) {
                    ...
                    errstream.str("");
                    errstream << "Token '" << token << "' invalid value for 'corr_mode' parameter." << endl;
                    error(errstream.str());
                }
            }
            ...
            while (iss >> token) {
                if (token.compare("fr") == 0) {
                    ...
                    errstream.str("");
                    errstream << "Token '" << token << "' invalid value for 'srt' parameter." << endl;
                    error(errstream.str());
                }
            }
            ...
            while ( iss >> token ) {
                if (token.compare("i") == 0) {
                    ...
                    errstream.str("");
                    errstream << "Token '" << token << "' invalid value for 'time_sampling' parameter." << endl;
                    error(errstream.str());
                }
            }
            ...
            if (wvr_corrected_data.compare("no") == 0)
                ...
                errstream.str("");
                errstream << "Token '" << wvr_corrected_data << "' invalid for 'wvr_corrected_data' parameter." << endl;
                error(errstream.str());
            }
        ...
        try {
            ...
            asdm::ASDMParseOptions parse = asdm::ASDMParseOptions().loadTablesOnDemand(true).checkRowUniqueness(checkRowUniqueness);
        ...
        //
        sdmbin::SDMBinData sdmBinData(ds, dsName);
        ...
        //
        casacore::Stokes::StokesTypes linearCorr[] = { casacore::Stokes::XX, casacore::Stokes::XY, casacore::Stokes::YX, casacore::Stokes::YY };
        casacore::Stokes::StokesTypes circularCorr[] = { casacore::Stokes::RR, casacore::Stokes::RL, casacore::Stokes::LR, casacore::Stokes::LL };
        ...
        try {
            ...
            for ( int i = 0; i < nPolarization; i++ ) {
                ...
                //vector<casacore::Stokes::StokesTypes> corrType;
                vector<int> corrType;
                StokesMapper stokesMapper;
                if (numCorr != 3) {
        ...
        static int ddIdx;      // not sure if this really needs to be static or not, but
                               // it was made static to be consistent with asdm2MS.cc usage
        ...
        try {
            ...
            for (int i = 0; i < nFeed; i++) {
                ...
                vector<std::complex<float> > polarization_response_ = CConverter::toVectorCF(r->getPolResponse());
                        ...
                        for (int j = 0 ; j < numSample; j++) { // ... must be expanded in numSample MS-Pointing rows.
                            ...
                            if ( r->isSampledTimeIntervalExists()) { //if sampledTimeInterval is present use its values.	           
                                ...
                                time_[iMSPointingRow] = ((double) timeInterval.at(j).getStart().get()) /
                                    ArrayTime::unitsInASecond + interval_[iMSPointingRow]/2;	  
...
#define OPT_ATTR_PAIR( rowPtr, AttributeName ) rowPtr -> is ## AttributeName ## Exists() ? make_pair ( true, rowPtr -> get ## AttributeName ().get()) : make_pair( false, 0.)        
        ...
        if (lazy) {
            ...
            for_each(report.begin(), report.end(), [&](const string &s){ this->warning(s); } ); 
    ...
    void sdm::info(const string& message) const {
        if (!verbose){
            return;
        }
     ...
     */
    void sdm::fillSpectralWindow(ASDM* ds_p, map<unsigned int, double>& effectiveBwPerSpwId_m, const string &telescopeName) {
            ...
            for(int ephemerisId: ephemerisId_s) {
                ...
                if (ephRow_v.size()==1) {
                    ...
                    for (unsigned int itab = 0; itab < tabulation_time_v.size(); itab++) {
                        ...
                        // RADVEL
                        if (radVelExists) {
                            radVelMS_v.push_back( evalPoly(radvel_coeff_v.size(), radvel_coeff_v, timeOrigin, tabulation_time) );
                        }
                    }
                } else {
     ...
     */
    int sdm::getNumBin(SpectralWindowRow *spwRow, const string &telescopeName) {
    ...
    static void fillSysPower_aux (const sdm &s, const vector<SysPowerRow *>& sysPowers, map<AtmPhaseCorrectionMod::AtmPhaseCorrection, ASDM2MSFiller*>& msFillers_m) {
        ...
        transform(sysPowers.begin(), sysPowers.end(), antennaId.begin(), [&](const SysPowerRow *r){return s.sysPowerAntennaId(r);});
        transform(sysPowers.begin(), sysPowers.end(), spectralWindowId.begin(), [&](const SysPowerRow *r){return s.sysPowerSpectralWindowId(r);});
        transform(sysPowers.begin(), sysPowers.end(), feedId.begin(), [&](const SysPowerRow *r){return s.sysPowerFeedId(r);});
        transform(sysPowers.begin(), sysPowers.end(), time.begin(), [&](const SysPowerRow *r){return s.sysPowerMidTimeInSeconds(r);});
        transform(sysPowers.begin(), sysPowers.end(), interval.begin(), [&](const SysPowerRow *r){return s.sysPowerIntervalInSeconds(r);});
            ...
            try {
                ...
                if (file_exists(uniqSlashes(asdmDirectory + "/SysPower.bin"))) {
                    ...
                    while (tsrSysPower.hasRows()) {
                        ...
                        if (sysPowers.size() > 0)
                            fillSysPower_aux(*this, sysPowerRows, msFillers_m);
                    }
                    tsrSysPower.close();
                }
    ...
    template<typename T> void v2oss( std::vector<T> v, ostringstream& oss, const std::string& oChar,
                                     const std::string& cChar, const std::string& sepChar) {
        oss << oChar;
        if (v.size() > 0) {
            oss << v[0];
            if (v.size() > 1) {
     ...
     *
     */
    void sdm::fillState(MainRow* r_p) {
        LOGENTER("fillState");
     ...
     */ 
    void sdm::fillMain( MainRow* r_p, sdmbin::SDMBinData &sdmBinData, const sdmbin::VMSData *vmsData_p, UvwCoords &uvwCoords,
                        std::map<unsigned int, double>& effectiveBwPerDD_m, bool complexData, bool mute,
                        bool ac_xc_per_timestamp, bool skipFirstTime ) {
    ...
    template<class T> void sdm::checkVectorSize( const string& vectorAttrName, const vector<T>& vectorAttr,
    ...
    // with 'functional programming' techniques.
    //
    int sdm::sysPowerAntennaId(const SysPowerRow* row) const {
        return row->getAntennaId().getTagValue();
    }

    int sdm::sysPowerSpectralWindowId(const SysPowerRow* row) const {
        return swIdx2Idx.at(row->getSpectralWindowId().getTagValue());
    }

    int sdm::sysPowerFeedId(const SysPowerRow* row) const {
        return row->getFeedId();
    }

    double sdm::sysPowerMidTimeInSeconds(const SysPowerRow* row) const {
        //if (isEVLA) {
