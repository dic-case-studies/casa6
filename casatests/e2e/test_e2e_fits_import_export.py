...
class regression_fits_import_export_test(unittest.TestCase):
    ...
    def tearDown(self):
        for i in list(mydict.keys()):
            os.remove("{}/{}{}".format(os.getcwd(),mydict[i][0],"exp.fits"))
            shutil.rmtree("{}/{}exp".format(os.getcwd(),mydict[i][0]))
            shutil.rmtree("{}/{}".format(os.getcwd(),mydict[i][0]))

        for listfile in os.listdir(os.getcwd()):
            if (listfile.endswith(".fits")) or (listfile.endswith(".last")):
    ...
    def test_regression(self):
        ...
        self.assertTrue(val)
def suite():
    return[regression_fits_import_export_test]
