...
class imregrid_test(unittest.TestCase):
    ...
    def test_asvelocity(self):
        ...
        shutil.copytree(ctsys.resolve(os.path.join(datapath,image)), image)
        shutil.copytree(ctsys.resolve(os.path.join(datapath,expected)), expected)
        myia = self._myia
        myia.open(expected)
    ...
    def test_regrid_galactic(self):
        ...
        myia = self._myia
        myia.open(ctsys.resolve(os.path.join(datapath,"ngc5921.clean.image")))
        csys = myia.coordsys()
        ...
        myia.open(ctsys.resolve(os.path.join(datapath,"gal_regrid.image")))
    ...
    def test_linear_overlap(self):
        ...
        myia = self._myia
        myia.open(ctsys.resolve(os.path.join(datapath,"lin_template.im")))
        csys = myia.coordsys().torecord()
        shape = myia.shape()
        myia.done()
        myia.open(ctsys.resolve(os.path.join(datapath,"lin_source.im")))
    ...
    def test_complex(self):
        ...
        myia = self._myia
        for i in (0, 1):
            myia.open(ctsys.resolve(os.path.join(datapath,"real1.im")))
            if i == 1:
                gg = myia.getchunk()
            ...
            rrg.done()
            myia.open(ctsys.resolve(os.path.join(datapath,"imag1.im")))
            if i == 1:
                gg = myia.getchunk()
                myia.fromarray("", gg, myia.coordsys().torecord(), type='d')
