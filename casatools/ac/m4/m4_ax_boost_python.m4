...
if test "x$BOOST_LIBRARY_PATH" != "x"; then
  LDFLAGS="-L$BOOST_LIBRARY_PATH $LDFLAGS"
fi
...
if test "$ac_cv_boost_python" = "yes"; then
  ...
  BOOSTLIBDIR=$BOOST_LIBRARY_PATH
  ax_boost_python_libraries=
  if [[ "$ax_python_lib" ]]; then
    ax_boost_python_libraries="$ax_boost_python_libraries `ls $BOOSTLIBDIR/lib$ax_python_lib.* 2> /dev/null`"
  fi
  if [[ "$ax_boost_python_lib" ]]; then
    ax_boost_python_libraries="$ax_boost_python_libraries `ls $BOOSTLIBDIR/lib$ax_boost_python_lib.* 2> /dev/null`"
  fi
  for axx_lib in $ax_boost_python_libraries `ls $BOOSTLIBDIR/libboost*.so* $BOOSTLIBDIR/libboost*.dylib* 2>/dev/null | grep python`; do
    [ax_lib_name=`echo $axx_lib | sed 's,.*/,,' | sed -e 's;^lib\(boost_python.*\)\.so.*$;\1;' -e 's;^lib\(boost_python.*\)\.dylib.*$;\1;' -e 's;^lib\(boost_python.*\)\.a.*$;\1;' `]
    AS_VAR_PUSHDEF([ax_Lib], [ax_cv_lib_$ax_lib_name''_BOOST_PYTHON_MODULE])dnl
    AC_CACHE_CHECK([whether $ax_lib_name is the correct library], [ax_Lib],
    [LIBS="-l$ax_lib_name $ax_boost_python_save_LIBS"
    AC_LINK_IFELSE([AC_LANG_PROGRAM([[
#include <boost/python/module.hpp>
BOOST_PYTHON_MODULE(test) { throw "Boost::Python test."; }]], [])],
        [AS_VAR_SET([ax_Lib], [yes])],
        [AS_VAR_SET([ax_Lib], [no])])])
    AS_VAR_IF([ax_Lib], [yes], [BOOST_PYTHON_LIB=$axx_lib break], [])
