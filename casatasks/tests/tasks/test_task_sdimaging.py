...
# https://casadocs.readthedocs.io/en/stable/api/tt/casatasks.single.sdimaging.html
#
#
##########################################################################
import copy
...
class sdimaging_unittest_base(unittest.TestCase):
    ...
    def _check_weight_image(self, imagename):
        ...
        try:
            for name in ['referencepixel', 'referencevalue', 'increment']:
                ...
                self.assertTrue(key in v0)
                self.assertTrue(key in v1)
                # CAS-12984 DO NOT TEST NUMERIC VALUE
                # wcs values are not exactly the same between
                # science and weight images generated by sdimaging
    ...
    def _checkstats(self, name, ref, compstats=None,
        ...
        for key in compstats:
            message = f"statistics '{key}' does not match: {stats[key]} (expected: {ref[key]})"
    ...
    def _check_beam(self, image, ref_beam):
        ...
        self.assertAlmostEqual(abs(min_asec - min_asec_ref) / max(min_asec_ref, 1.e-12), 0.,
                               places=3,
...
class sdimaging_test0(sdimaging_unittest_base):
    ...
    def test010(self):
        ...
        sdimaging(infiles=self.rawfile, outfile=self.outfile, intent='', cell=self.cell,
                  imsize=self.imsize, phasecenter=self.phasecenter.replace('J2000', 'J3000'),
                  minweight=self.minweight0)
        sdimaging(infiles=self.rawfile, outfile=refimage, intent='', cell=self.cell,
                  imsize=self.imsize, phasecenter=self.phasecenter, minweight=self.minweight0)
        self._checkfile(self.outfile)
        self._check_weight_image(self.outfile)
        self._checkfile(refimage)
        self._check_weight_image(refimage)
    ...
    def test011(self):
        ...
        # casatasks throws an exception, CASA5 return False
        self.assertRaises(
            Exception, sdimaging, infiles=self.rawfile, outfile=self.outfile, intent='',
            cell=self.cell, imsize=self.imsize, phasecenter=self.phasecenter,
...
class sdimaging_test_flag(sdimaging_unittest_base):
    ...
    def testFlag02(self):
        res = sdimaging(infiles=self.rawfile, outfile=self.outfile, intent="",
                        width=10, gridfunction=self.gridfunction,
                        cell=self.cell, imsize=self.imsize,
        ...
        for i in range(len(self.x_range)):
            for j in range(len(self.y_range)):
                for k in range(len(self.f_range)):
                    self._checkvalue(
                        self.outfile, False, self.x_range[i], self.y_range[j],
