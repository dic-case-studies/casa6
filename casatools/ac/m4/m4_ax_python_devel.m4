...
#   'Python.h'. It provides the $(PYTHON_CPPFLAGS) and $(PYTHON_LIB) output
...
variables PYTHON_CPPFLAGS, PYTHON_LIB, PYTHON_SITE_PKG,
...
$ac_distutils_result])
	...
	if test -z "$PYTHON_INCLUDE_PATH"; then
		...
		if test -n "${python_path}"; then
			if test "${plat_python_path}" != "${python_path}"; then
				python_path="$python_path $plat_python_path"
			else
				python_path="$python_path"
			fi
		fi
		PYTHON_INCLUDE_PATH=$python_path
	fi
...
#			PYTHON_LIB="-L$ac_python_libdir -l$ac_python_library"
            if [[ ! -d $ac_python_libdir ]]; then
                AC_MSG_ERROR([python library ($ac_python_libdir) is not a directory...])
            fi
            PYTHON_LIB=
            for ax_dir in `ls $ac_python_libdir/lib$ac_python_library.*`; do
                if [[ ! -h $ax_dir ]]; then
                    if [[ -z "$PYTHON_LIB" ]]; then
                        PYTHON_LIB=$ax_dir
                    else
                        PYTHON_LIB="$PYTHON_LIB $ax_dir"
                    fi
                fi
            done
		else
            ...
            PYTHON_LIB=
            for ax_dir in `ls $ac_python_libdir/libpython$ac_python_version.*`; do
                if [[ ! -h $ax_dir ]]; then
                    if [[ -z "$PYTHON_LIB" ]]; then
                        PYTHON_LIB=$ax_dir
                    else
  ...
  dynamic libraries enabled, or try setting PYTHON_LIB by hand.
			])
		fi
	fi
