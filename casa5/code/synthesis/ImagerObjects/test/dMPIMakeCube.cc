...
int main(int argc, char **argv)
{
	...
	if(casa::applicator.isController())
 {
	...
	cerr << "shapes " <<  imstor->residual()->shape() <<  " " <<  imstor->psf()->shape() <<  " "  <<  endl;

        String cimagename="Complex.image";
        //For some reason cannot open a new paged image with UserNoread directly
    {
    
        PagedImage<Complex> cImage(imstor->residual()->shape(), imstor->residual()->coordinates(), cimagename);
    }
	...
	Bool allDone(false);
        Int typeofImage=Int(refim::FTMachine::OBSERVED);
	...
	//if(casa::applicator.isController()) {
        for (Int k=0; k < numchan; ++k) {
			...
			//Type of image #5
                        applicator.put(typeofImage);
			// weighting parameters #6
			applicator.put(weightRecord);
                        // complex imagename #7
                        applicator.put(cimagename);
