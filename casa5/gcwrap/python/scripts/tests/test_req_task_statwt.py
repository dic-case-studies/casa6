...
if th.is_casa6():
    ...
    datadir = casatools.ctsys.resolve(subdir)
else:
...
# datadir = os.environ.get('CASAPATH').split()[0] \
#    + '/data/regression/unittest/statwt/'
...
class statwt_test(unittest.TestCase):
        ...
        for combine in ["corr", "corr,field"]:
            for i in [0, 1]:
                ...
                if i == 0:
                    ...
                    statwt(dst, timebin=timebin, combine=combine)
                #self._check_weights(
                #    dst, row_to_rows, 'c', None, True, None, None
                #)
                self.compare(dst, ref)
                shutil.rmtree(dst)
            ...
            for i in [0, 1]:
                ...
                if i == 0:
                    ...
                    if statalg == "cl":
                        ...
                        self.assertTrue(
                            myms.statwt(statalg=statalg, center="median",
                            lside=False)
                        )
                    elif statalg == "bogus":
                        self.assertRaises(
                            Exception, myms.statwt, statalg=statalg
                        )
                    myms.done()
                else:
                    if statalg == "cl":
                        ...
                        self.assertTrue(
                            statwt(
                                vis=dst, statalg=statalg, zscore=5, maxiter=3
                            )
                        )
                    elif statalg == "h":
                        self.assertTrue(
                        ...
                        self.assertTrue(
                            statwt(
                                vis=dst, statalg=statalg, center="median",
                                lside=False
                            )
                        )
                    elif statalg == "bogus":
    ...
    def test_preview(self):
        """ Test preview mode"""
        dst = "ngc5921_statwt_ngc5921.split.preview.ms"
        ...
        for i in [0, 1]:
            ...
            if i == 0:
                myms.open(dst, nomodify=False)
                myms.statwt(
                    timebin=timebin, combine=combine, wtrange=wtrange,
                    preview=preview
                )
                myms.done()
            else:
