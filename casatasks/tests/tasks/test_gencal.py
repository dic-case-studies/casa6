...
class TestJyPerK(unittest.TestCase):
    ...
    """
    temporary_directory = 'temporary_directory_for_jyperk'
    
    ms_key = 'uid___A002_X85c183_X36f.ms'
    vis = os.path.join(temporary_directory, ms_key)

    # The caltable is generated by each gencal task.
    caltable = os.path.join(temporary_directory, 'generated_caltable_by_gencal.cal')
    ...
    @classmethod
    def setUpClass(cls):
        if os.path.exists(cls.temporary_directory):
            shutil.rmtree(cls.temporary_directory)

        os.mkdir(cls.temporary_directory)
