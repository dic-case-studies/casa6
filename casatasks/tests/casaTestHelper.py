...
class Weblog:
    ...
    def generate_table_row(self, counter, test, description, runtime, status_color):
        html.write("<tr>" + "\n")
        html.write('<td class="tg-0lax">{}</td>'.format(counter) + '\n')
    ...
    def generate_status_table(self, dictionary, show_passed):
        ...
        html.write('<tr>' + '\n')
        html.write('<th class="tg-0lax">#</th>' + '\n')
        ...
        if self.all_passed:
            html.write("<tr>" + "\n")
            html.write('<td class="tg-0lax">All Test Passed</td>' + '\n')
            html.write("</tr>" + "\n")
        html.write('</table>' + '\n')
...
def check_ret( summ,correctres,correctmod,epsilon = 0.05, testname ="check_ret"):
    pstr = ''
...
def check_val(val, correctval, valname='Value', exact=False, epsilon=0.05, testname = "check_val"):
    pstr = ''
    out = True
...
def check_val_less_than(val, bound, valname='Value',testname ="check_val_less_than"):
    pstr = ''
    out = True
    if numpy.isnan(val) or numpy.isinf(val):
        out = False
...
def check_val_greater_than(val, bound, valname='Value',testname ="check_val_greater_than"):
    pstr = ''
    out = True
    if numpy.isnan(val) or numpy.isinf(val):
        out = False
...
def check_ims(imlist, truth, testname="check_ims"):
    pstr = ''
    imex = []
    out = True
...
def check_keywords(imlist, testname="check_keywords"):
    """
...
def check_pix_val(imname, theval=0, thepos=[0, 0, 0, 0], exact=False, epsilon=0.05, testname="check_pix_val"):
    pstr = ''
...
def check_pixmask(imname, theval=True, thepos=[0, 0, 0, 0], testname="check_pixmask"):
    pstr = ''
...
def check_ref_freq(imname, theval=0, epsilon=0.05, testname="check_ref_freq"):
    pstr = ''
    retres = True
    ia.open(imname)
    csys = ia.coordsys()
...
def check_tabcache(tabcache, testname="check_tabcache"):
    pstr = ''
    if tabcache == True:
...
def check_stopcode(stopcode, ret, testname="check_stopcode"):
    pstr = ''
    if stopcode != None:
...
def checkall(ret=None, peakres=None, modflux=None, iterdone=None, nmajordone=None, imgexist=None, imgexistnot=None, imgval=None, imgvalexact=None, imgmask=None, tabcache=True, stopcode=None, reffreq=None, epsilon=0.05):
    ...
    pstr += check_stopcode(stopcode, ret)
