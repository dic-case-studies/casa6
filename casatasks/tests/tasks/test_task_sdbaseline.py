...
class sdbaseline_basicTest(sdbaseline_unittest_base):
    ...
    def test003(self):
        ...
        # sdbaseline returns None if it runs successfully
        self.assertEqual(result, None, msg="The task returned '" +
                         str(result) + "' instead of None")
        # self._compareBLparam(outfile+"_blparam.txt",self.blrefroot+tid)
        results = self._getStats(outfile)
        print(self._getStats(outfile))
    ...
    def test062(self):
        ...
        # First run
        sdbaseline(infile=infile, outfile=outfile, overwrite=overwrite,
                   datacolumn=datacolumn, blmode=blmode, blformat=blformat)
        ...
        overwrite = True
        sdbaseline(infile=infile, outfile=outfile, overwrite=overwrite,
                   datacolumn=datacolumn, blmode=blmode, bltable=bltable)
        ...
        for ext in ['txt', 'bltable']:
            ...
            blparamfile_backup = blparamfile + '.backup'
            if ext == 'txt':
                self.assertTrue(filecmp.cmp(blparamfile, blparamfile_backup),
                                msg='{} changed'.format(blparamfile))
            elif ext == 'bltable':
    ...
    def test070(self):
        ...
        datacolumn = 'float_data'
        dosubtract = False
        blformats = ['text', 'csv', 'table', ['text', 'table'],
                     ['text', ''], ['text', 'csv', 'table']]
    ...
    def test080(self):
        ...
        try:
            sdbaseline(infile=infile, outfile=outfile, dosubtract=dosubtract,
                       overwrite=overwrite, datacolumn=datacolumn)
        except Exception as e:
...
class sdbaseline_multi_IF_test(sdbaseline_unittest_base):
    ...
    def test200(self):
        ...
        result = sdbaseline(infile=infile, maskmode=mode,
                            outfile=outfile, blfunc=blfunc, order=order)
        self.assertEqual(result, None, msg="The task returned '" +
                         str(result) + "' instead of None")
    ...
    def _checkBltableVar(self, outms, bltable, blparam, option):
        ...
        try:
            for i in range(npol * tb.nrows()):
                ...
                self._checkValue(float(blparam['cthre'][i]), tb.getcell(
                    'CLIP_THRESHOLD', irow)[ipol][0], 1.0e-6)
                ...
                lthre = 5.0 if ((blparam['lthre'][i] == '')
                                or not uself) else float(blparam['lthre'][i])
                ...
                ledge = 0 if ((blparam['ledge'][i] == '')
                              or not uself) else int(blparam['ledge'][i])
                self.assertEqual(ledge, tb.getcell('LF_EDGE', irow)[ipol][0])
                redge = 0 if ((blparam['redge'][i] == '')
                              or not uself) else int(blparam['redge'][i])
    ...
    def _checkValue(self, ref, out, tol=1.0e-02):
        ...
        if (abs(ref) > tol) or (abs(out) > tol):
            if ref != 0.0:
                ...
                rel = abs(out - ref)
            self.assertTrue((rel < tol), msg='the output (' + str(out) +
                            ') differs from reference (' + str(ref) + ')')
...
class sdbaseline_variableTest(sdbaseline_unittest_base):
    ...
    * edge flagging
    ...
    def testVariable01(self):
        ...
        self._refetch_files([infile, self.paramfile], self.datapath)
        self._run_test(infile, self.refstat0, flag_spec=[
                       (0, 0)], blparam=self.paramfile, datacolumn=self.column)
    ...
    def testVariable02(self):
        ...
        self._refetch_files([infile, self.paramfile], self.datapath)
        self._run_test(infile, self.refstat0, flag_spec=[
                       (0, 0), (1, 1)], blparam=self.paramfile, datacolumn=self.column)
...
Basic unit tests for task sdbaseline. No interactive testing.
    ...
    def setUp(self):
        ...
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_cspline_txt),
                        self.bloutput_cspline_txt)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_cspline_csv),
                        self.bloutput_cspline_csv)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_variable_txt),
                        self.bloutput_variable_txt)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_variable_csv),
                        self.bloutput_variable_csv)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_csv),
                        self.bloutput_sinusoid_csv)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_txt),
                        self.bloutput_sinusoid_txt)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwn012_rejwn0_txt),
                        self.bloutput_sinusoid_addwn012_rejwn0_txt)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwn012_rejwn02_txt),
                        self.bloutput_sinusoid_addwn012_rejwn02_txt)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwn012_rejwn1_txt),
                        self.bloutput_sinusoid_addwn012_rejwn1_txt)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwn012_rejwn0_csv),
                        self.bloutput_sinusoid_addwn012_rejwn0_csv)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwn012_rejwn02_csv),
                        self.bloutput_sinusoid_addwn012_rejwn02_csv)
        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwn012_rejwn1_csv),
                        self.bloutput_sinusoid_addwn012_rejwn1_csv)

        shutil.copyfile(os.path.join(self.datapath, self.bloutput_sinusoid_addwnGt4000_rejwn4005_txt),
                        self.bloutput_sinusoid_addwnGt4000_rejwn4005_txt)
    ...
    def check_bloutputparam_csv(self, bloutputfile, ref_all):
        with open(bloutputfile, 'r') as file:
            ...
            self.assertEqual(
                ref_all, list_all, msg='parameter values of the output csv file are not equivalent to referece values!')
    ...
    def test000(self):
        ...
        self.assertEqual(diff_value, 0, msg=bloutput[0] +
                         ' is not equivalent to ' + self.bloutput_poly_csv)

        diff_value = os.system('diff test.txt ' + self.bloutput_poly_txt)
        self.assertEqual(diff_value, 0, msg=bloutput[1] +
                         ' is not equivalent to ' + self.bloutput_poly_txt)
    ...
    def test001(self):
        ...
        diff_value = os.system('diff test.csv ' + self.bloutput_poly_csv)
        self.assertEqual(diff_value, 0,
            msg=bloutput[1] + ' is not equivalent to ' + self.bloutput_poly_csv)

        diff_value = os.system('diff test.txt ' + self.bloutput_poly_txt)
        self.assertEqual(diff_value, 0,
            msg=bloutput[0] + ' is not equivalent to ' + self.bloutput_poly_txt)
    ...
    def test002(self):
        ...
        diff_value = os.system('diff test.csv ' + self.bloutput_poly_csv)
        self.assertEqual(diff_value, 0, msg=bloutput[2] +
                         ' is not equivalent to ' + self.bloutput_poly_csv)
    ...
    def test003(self):
        ...
        self.assertEqual(result_exist, True, msg=self.infile +
                         '_blparam.bltable' + ' does not exist!')
    ...
    def test004(self):
        ...
        diff_value = os.system('diff ' + self.infile + '_blparam.txt' +
                               ' ' + self.bloutput_poly_txt)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.txt' +
                         ' is not equivalent to ' + self.bloutput_poly_txt)

        diff_value = os.system('diff ' + self.infile + '_blparam.csv' +
                               ' ' + self.bloutput_poly_csv)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.csv' +
                         ' is not equivalent to ' + self.bloutput_poly_csv)
    ...
    def test013(self):
        ...
        if(os.path.exists('test.csv') == True):
            ...
            self.assertEqual(
                diff_value, 0, msg=bloutput[2] + 'is not equivalent to ' + self.bloutput_variable_csv)

        if(os.path.exists('test.txt') == True):
            diff_value = os.system('diff test.txt ' + self.bloutput_variable_txt)
            self.assertEqual(
                diff_value, 0, msg=bloutput[1] + 'is not equivalent to ' + self.bloutput_variable_txt)
    ...
    def test016(self):
        ...
        self.assertEqual(diff_value, 0, msg=bloutput[0] +
                         ' is not equivalent to ' + self.bloutput_cspline_csv)

        diff_value = os.system('diff test.txt ' + self.bloutput_cspline_txt)
        self.assertEqual(diff_value, 0, msg=bloutput[1] +
                         ' is not equivalent to ' + self.bloutput_cspline_txt)
    ...
    def test019(self):
        ...
        diff_value = os.system('diff test.csv ' + self.bloutput_cspline_csv)
        self.assertEqual(diff_value, 0,
            msg=bloutput[2] + 'is not equivalent to ' + self.bloutput_cspline_csv)

        diff_value = os.system('diff test.txt ' + self.bloutput_cspline_txt)
        self.assertEqual(diff_value, 0,
            msg=bloutput[1] + 'is not equivalent to ' + self.bloutput_cspline_txt)
    ...
    def test020(self):
        ...
        result_exist = os.path.exists(self.infile + '_blparam.bltable')
        self.assertEqual(result_exist, True,
            msg=self.infile + '_blparam.bltable' + ' does not exist!')
        ...
        diff_value = os.system('diff ' + self.infile + '_blparam.csv' +
                               ' ' + self.bloutput_cspline_csv)
        self.assertEqual(diff_value, 0,
            msg=bloutput[0] + ' is not equivalent to ' + self.bloutput_cspline_csv)

        diff_value = os.system('diff ' + self.infile + '_blparam.txt' +
                               ' ' + self.bloutput_cspline_txt)
        self.assertEqual(diff_value, 0,
            msg=self.infile + '_blparam.txt' + ' is not equivalent to ' + self.bloutput_cspline_txt)
    ...
    def test026(self):
        ...
        if(os.path.exists('test.csv') == True):
            diff_value = os.system('diff test.csv ' + self.bloutput_cspline_csv)
            self.assertEqual(
                diff_value, 0, msg=bloutput[1] + 'is not equivalent to ' + self.bloutput_cspline_csv)
    ...
    def test029(self):
        ...
        if(os.path.exists('test.csv') == True):
            diff_value = os.system('diff test.csv ' + self.bloutput_variable_csv)
            self.assertEqual(
                diff_value, 0, msg=bloutput[0] + 'is not equivalent to ' + self.bloutput_variable_csv)
    ...
    def test030(self):
        ...
        if(os.path.exists('test.csv') == True):
            diff_value = os.system('diff test.csv ' + self.bloutput_variable_csv)
            self.assertEqual(
                diff_value, 0, msg=bloutput[1] + 'is not equivalent to ' + self.bloutput_variable_csv)

        if(os.path.exists('test.txt') == True):
            diff_value = os.system('diff test.txt ' + self.bloutput_variable_txt)
            self.assertEqual(
                diff_value, 0, msg=bloutput[0] + 'is not equivalent to ' + self.bloutput_variable_txt)
    ...
    def test032(self):
        ...
        diff_value = os.system('diff test.csv ' + self.bloutput_variable_csv)
        self.assertEqual(diff_value, 0, msg=bloutput[2] +
                         'is not equivalent to ' + self.bloutput_variable_csv)

        diff_value = os.system('diff test.txt ' + self.bloutput_variable_txt)
        self.assertEqual(diff_value, 0, msg=bloutput[1] +
                         'is not equivalent to ' + self.bloutput_variable_txt)
    ...
    def test038(self):
        ...
        #diff_value=os.system('diff ' + self.infile + '_blparam.txt ' + self.bloutput_variable_txt)
        #self.assertEqual(diff_value, 0,
        #    msg=self.infile + '_blparam.txt' + 'is not equivalent to ' + self.bloutput_variable_txt)
    ...
    def test042(self):
        ...
        if(os.path.exists('test.csv') == True):
            ...
            self.assertEqual(
                diff_value, 0, msg=bloutput[0] + 'is not equivalent to ' + self.bloutput_sinusoid_csv)

        if(os.path.exists('test.txt') == True):
            diff_value = os.system('diff test.txt ' + self.bloutput_sinusoid_txt)
            self.assertEqual(
                diff_value, 0, msg=bloutput[1] + 'is not equivalent to ' + self.bloutput_sinusoid_txt)
    ...
    def test043(self):
        ...
        if(os.path.exists('test.csv') == True):
            diff_value = os.system('diff test.csv ' + self.bloutput_sinusoid_csv)
            self.assertEqual(
                diff_value, 0, msg=bloutput[1] + 'is not equivalent to ' + self.bloutput_sinusoid_csv)

        if(os.path.exists('test.txt') == True):
            diff_value = os.system('diff test.txt ' + self.bloutput_sinusoid_txt)
            self.assertEqual(
                diff_value, 0, msg=bloutput[0] + 'is not equivalent to ' + self.bloutput_sinusoid_txt)
    ...
    def test045(self):
        ...
        diff_value = os.system('diff test.csv ' + self.bloutput_sinusoid_csv)
        self.assertEqual(diff_value, 0, msg=bloutput[2] +
                         'is not equivalent to ' + self.bloutput_sinusoid_csv)

        diff_value = os.system('diff test.txt ' + self.bloutput_sinusoid_txt)
        self.assertEqual(diff_value, 0, msg=bloutput[1] +
                         'is not equivalent to ' + self.bloutput_sinusoid_txt)
    ...
    def test0124(self):
        ...
        result = self.run_test(blfunc=blfunc, blformat=blformat,
                               bloutput=bloutput, addwn=addwn, rejwn=rejwn)
        self.assertEqual(result, None, msg="The task returned '" +
                         str(result) + "' instead of None")
        ...
        diff_value = os.system('diff ' + self.infile + '_blparam.csv ' +
                               self.bloutput_sinusoid_addwn012_rejwn0_csv)
        self.assertEqual(diff_value, 0,
            msg=self.infile + '_blparam.csv' + 'is not equivalent to ' + self.bloutput_sinusoid_addwn012_rejwn0_csv)

        diff_value = os.system('diff ' + self.infile + '_blparam.txt ' +
                               self.bloutput_sinusoid_addwn012_rejwn0_txt)
        self.assertEqual(diff_value, 0,
            msg=self.infile + '_blparam.txt' + 'is not equivalent to ' + self.bloutput_sinusoid_addwn012_rejwn0_txt)
    ...
    def test0125(self):
        ...
        diff_value = os.system('diff ' + self.infile + '_blparam.csv ' +
                               self.bloutput_sinusoid_addwn012_rejwn02_csv)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.csv' +
                         'is not equivalent to ' + self.bloutput_sinusoid_addwn012_rejwn02_csv)

        diff_value = os.system('diff ' + self.infile + '_blparam.txt ' +
                               self.bloutput_sinusoid_addwn012_rejwn02_txt)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.txt' +
                         'is not equivalent to ' + self.bloutput_sinusoid_addwn012_rejwn02_txt)
    ...
    def test0126(self):
        ...
        diff_value = os.system('diff ' + self.infile + '_blparam.csv ' +
                               self.bloutput_sinusoid_addwn012_rejwn1_csv)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.csv' +
                         'is not equivalent to ' + self.bloutput_sinusoid_addwn012_rejwn1_csv)

        diff_value = os.system('diff ' + self.infile + '_blparam.txt ' +
                               self.bloutput_sinusoid_addwn012_rejwn1_txt)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.txt' +
                         'is not equivalent to ' + self.bloutput_sinusoid_addwn012_rejwn1_txt)
    ...
    def test0127(self):
        ...
        result = self.run_test(blfunc=blfunc, blformat=blformat, bloutput=bloutput,
                               addwn=addwn, rejwn=rejwn, spw=spw, applyfft=applyfft)
        self.assertEqual(result, None, msg="The task returned '" +
                         str(result) + "' instead of None")
        ...
        diff_value = os.system('diff ' + self.infile + '_blparam.txt ' +
                               self.bloutput_sinusoid_addwnGt4000_rejwn4005_txt)
        self.assertEqual(diff_value, 0, msg=self.infile + '_blparam.txt' +
                         'is not equivalent to ' + self.bloutput_sinusoid_addwnGt4000_rejwn4005_txt)
    ...
    def _test_result(self, msname, sel_param, dcol, atol=1.e-5, rtol=1.e-5, applymode=False):
        ...
        try:
            if not applymode:  # normal fit
                self.assertEqual(tb.nrows(), len(
                    rowids), "Row number is wrong %d (expected: %d)" % (tb.nrows(), len(rowids)))
            ...
            for out_row in range(len(rowids)):
                ...
                if not poltest:
                    self.assertEqual(sp.shape[0], len(polids), "Number of pol is wrong in row=%d:  %d (expected: %d)" % (
