...
namespace casa { //# NAMESPACE CASA - BEGIN
  ...
  void SIMapperCollection::cleanupTempFiles(const String& mess) 
  {
    for(Int mapperid=0;mapperid<nMappers();mapperid++ )
      {
	itsMappers[mapperid]->getFTM2(True)->cleanupTempFiles(mess);
	itsMappers[mapperid]->getFTM2(False)->cleanupTempFiles(mess);
      }
