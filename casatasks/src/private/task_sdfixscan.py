...
class sdfixscan_worker(sdutil.sdtask_interface):
    ...
    def __execute_press(self):
        ...
        self.convimage = self.image.convolve2d(outfile=self.tmppolyname,
                                               major=bmajor, minor=bminor, pa=pa,
                                               overwrite=True)
        self.convimage.done()
        ...
        if self.direction == 0.0:
            ...
            raise Exception(
                "Sorry, the task don't support inclined scan "
                "with respect to horizontal or vertical axis, right now.")
        ...
        # resultdic = self.convimage.fitprofile(model=self.tmppolyname, axis=fitaxis,
        #                    poly=self.numpoly, ngauss=0, multifit=True, gmncomps=0 )
        self.__polynomial_fit_model(image=self.tmpmskname,
                                    model=self.tmppolyname, axis=fitaxis, order=self.numpoly)
    ...
    def __execute_basket_weaving(self):
            ...
            for i in range(nfile):
                ...
                for iaxis2 in range(imshape[2]):
                    for iaxis3 in range(imshape[3]):
                        pixmsk = self.realimage.getchunk([0, 0, iaxis2, iaxis3],
        ...
        for i in range(nfile):
            ...
            for iaxis2 in range(imshape[2]):
                for iaxis3 in range(imshape[3]):
                    pixval = self.realimage.getchunk([0, 0, iaxis2, iaxis3],
                                                     [nx - 1, ny - 1, iaxis2, iaxis3])
            ...
            for iaxis3 in range(imshape[3]):
                ...
                for i in range(nfile):
                    ...
                    pixval = self.realimage.getchunk(
                        [0, 0, ichan, iaxis3], [nx - 1, ny - 1, ichan, iaxis3]) \
                        + self.imagimage.getchunk([0, 0, ichan, iaxis3],
                                                  [nx - 1, ny - 1, ichan, iaxis3]) * 1.0j
                    pixval = pixval.reshape((nx, ny))
        ...
        for ichan in range(imshape[2]):
            for iaxis3 in range(imshape[3]):
                pixval = self.realimage.getchunk([0, 0, ichan, iaxis3],
                                                 [nx - 1, ny - 1, ichan, iaxis3]) \
                    + self.imagimage.getchunk([0, 0, ichan, iaxis3],
                                              [nx - 1, ny - 1, ichan, iaxis3]) * 1.0j
                pixval = pixval.reshape((nx, ny))
