...
class SimpleTsysFiller( TsysFillerBase ):
    ...
    def __init__( self, filename, ifno, skip_channelaveraged=False ):
        ...
        self.ifno = ifno
        print('IFNO to be processed: {}'.format(self.ifno))
        self.skip_channelaveraged = skip_channelaveraged
        if skip_channelaveraged is True:
            print('SimpleTsysFiller: skip channel averaged spws ({})'.format(self.ifno + 1))
    ...
    def fillScanAveragedTsys( self ):
        ...
        spectral data.
        """
        print('POLNO={},BEAMNO={}'.format(self.polno,self.beamno if self.beamno is not None else 'all'))
        srctype = [10,11]
...
class TsysFiller( TsysFillerBase ):
    ...
    def __init__( self, filename, specif, tsysif=None, extrap=False, skip_channelaveraged=False ):
        ...
        self.skip_channelaveraged = skip_channelaveraged
        if skip_channelaveraged is True:
            print('TsysFiller: skip channel averaged spws ({})'.format(self.specif + 1))
        if tsysif is None:
            self.tsysif = None
            self.abctsys = None
        ...
        print('spectral IFNO {}: corresponding Tsys IFNO is {}'.format(self.specif,self.tsysif)) 
